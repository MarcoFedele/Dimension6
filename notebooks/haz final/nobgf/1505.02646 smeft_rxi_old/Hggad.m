...
diag = diag/.{MH->Sqrt[h], MW->Sqrt[w], MZ->Sqrt[z], MghWm->Sqrt[w], MghWp->Sqrt[w] };
...
total = total /. {cB -> g1^2 cB, cW -> gw^2 cW, cWB -> g1 gw cWB} /. cB -> c\[Gamma]\[Gamma] + cWB - cW;
...
total = Collect[total
        ...
        , Simplify];
total = total /. 2 lam vev^2 -> h /. vev^2*gw^2/4 -> w;
total = total /. g1^2 gw^2 -> e^2 (g1^2 + gw^2);
...
If[!MatchQ[Coefficient[totale, AAAA], 0] ,
   ...
   Print[""];
   Table[Print["Entry relative to ", c, " : ", Expand[Coefficient[-2*ad/4/e^2, c]] ,"\n"],{c,{c\[Gamma]\[Gamma],cB,cW,cWB}}],
...
If[!MatchQ[Coefficient[total0, AAAA], 0] ,
   ...
   ft = Collect[ Coefficient[total0/4, AAAA], {e, c\[Gamma]\[Gamma], cW, cWB,
