(* Here I generate the diagrams for h -> photon photon* in the SM, using the background field method and
...
(* Here I select only the O(LAMBDA^0) terms, and throw away BSM terms or further order terms.
...
(* Here I print the diagrams that I will compute in Haast_included.pdf, and the diagrams that I will
   not compute in Haast_excluded.pdf. *)

Print["Diagrams included: ",finaldiag];
Print["Total number of diagrams included: ",Length[diag],"/",Length[amp0]];
Paint[DiagramExtract[ins1, finaldiag], DisplayFunction -> (Export["Haast_included.pdf",#]&), ColumnsXRows ->{3,4}];
Print["Included diagrams printed.\n"];
If[Length[DiagramComplement[ins1, DiagramExtract[ins1, finaldiag]]] != 0,
   Paint[DiagramComplement[ins1, DiagramExtract[ins1, finaldiag]], DisplayFunction -> (Export["Haast_excluded.pdf",#]&), ColumnsXRows ->{3,4}];
   Print["Excluded diagrams printed."]
   ];

(* Here I store the result I obtained so far in the file Haast1.res. I have to do so, since I have to quit
   the kernel I am working on before loading the packages needed to go on, which is in conflict with
   FeynArts. *)

FILE = NotebookDirectory[]<>"Haast1.res";
