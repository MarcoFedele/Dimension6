...
decayKinematics := {sp[q1,q1] -> AA, sp[Ep1,q1] -> 0}
...
Print["\nStarting to compute matrix traces.\n"];
...
Do[
   ...
   temp1 = FixedPoint[(Expand[#] /. CRF3)&, temp1];
   temp1 = FixedPoint[(Expand[#] /. CRF4)&, temp1];
   temp1 = FixedPoint[(Expand[#] /. CRF5)&, temp1];
...
Diag = Diag /. SumOver[Index[Colour,3],3] -> Nc;
 
(* Here I store the result I obtained so far in the file zz2.res. I have to do so, since I have to quit
  the kernel I am working on before loading FeynCalc, which is in conflict with Tracer. *)

FILE = NotebookDirectory[]<>"zz2.res";
