(* Here I generate the diagrams for h -> 2 photons in the SMEFT, using the background field method and
...
top = CreateTopologies[1, 2 -> 1, Adjacencies -> {3, 4, 5}, ExcludeTopologies -> {Internal, Tadpoles} ];

ins1 = InsertFields[top,
                    process1,
                    Model -> "Mine/1505.02646.bgf.3",
                    GenericModel -> "Mine/1505.02646.bgf.3",
                    ExcludeParticles -> {F, V[1]},
...
(* Here I select only the O(1/LAMBDA^2) terms, and throw away SM terms or further order terms.
   The finaldiag array will contain the number of each of the original diagrams that I am really
   going to compute. *)

diag = {};
finaldiag = {};
Do[check = Coefficient[amp1[[i]], LAMBDA, -2];
   If[!MatchQ[check, 0],
      diag = Append[diag, check/LAMBDA^2];
      finaldiag = Append[finaldiag, i]
      ]
   ,{i, Length[amp1]}];

(* Here I print the diagrams that I will compute in Haa_included.pdf, and the diagrams that I will
   not compute in Haa_excluded.pdf. *)

Print["Diagrams included: ",finaldiag];
Print["Total number of diagrams included: ",Length[diag],"/",Length[amp0]];
Paint[DiagramExtract[ins1, finaldiag], DisplayFunction -> (Export["Haa_included.pdf",#]&), ColumnsXRows ->{3,4}];
Print["Included diagrams printed.\n"];
If[Length[DiagramComplement[ins1, DiagramExtract[ins1, finaldiag]]] != 0,
   Paint[DiagramComplement[ins1, DiagramExtract[ins1, finaldiag]], DisplayFunction -> (Export["Haa_excluded.pdf",#]&), ColumnsXRows ->{3,4}];
   Print["Excluded diagrams printed."]
   ];

(* Here I store the result I obtained so far in the file Haa1.res. I have to do so, since I have to quit
   the kernel I am working on before loading the packages needed to go on, which is in conflict with
   FeynArts. *)

FILE = NotebookDirectory[]<>"Haa1.res";
