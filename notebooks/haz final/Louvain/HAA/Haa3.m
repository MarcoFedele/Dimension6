...
(* Here I define some kinematic relations. N.B. with lower case letters (z,w,h) I refer to
...
diagprep = diagprep /. cB -> c\[Gamma]\[Gamma] + cWB - cW;

(* Here i perform the reduction to scalar integrals. At lines 69-70 I write explicitly
   the poles (i.e. terms O(1/\[Epsilon])); therefore, starting from line 71, all B0[___]
...
result = Collect[Total[diagfin]
                 ,{\[Epsilon], c\[Gamma]\[Gamma], cW, cWB, vev, A^\[Alpha]\[Beta], g^\[Alpha]\[Beta], B0[___], C0[___]}
                 , Simplify];

result = Collect[result /. {h->2lam vev^2, w->(vev*gw/2)^2, 1/(g1^2 + gw^2)->e^2/(g1^2 gw^2)}
                 ,{A^\[Alpha]\[Beta], \[Epsilon], c\[Gamma]\[Gamma], cW, cWB, vev, B0[___], C0[___], Log[_]},
                 Simplify] //. {lam vev^2 -> h/2, gw^2 vev^2 -> 4 w} ;
