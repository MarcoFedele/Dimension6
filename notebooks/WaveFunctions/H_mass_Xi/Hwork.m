...
total = Coefficient[Total[diag], q1, 2] /.{A0[m_] -> m/\[Epsilon] - m Log[m] + m, B0[x_, m1_, m2_] -> 1/\[Epsilon] + B0[x, m1, m2]} //. Reorder;

total = Normal[Series[total, {\[Epsilon], 0, 0}]];

total = total /. cw -> gw/Sqrt[(g1^2 + gw^2)] /. B0[h, w GaugeXi[W], w] -> B0[h, w , w GaugeXi[W]] /. B0[h, z GaugeXi[Z], z] -> B0[h, z, z GaugeXi[Z]];

total = Collect[total /. w -> gw^2/2 vev^2 /. z -> (g1^2 + gw^2)/2 vev^2,{\[Epsilon], A0[_], B0[___]},Simplify] /. gw^2/2 vev^2 -> w /. (g1^2 + gw^2)/2 vev^2 -> z;
