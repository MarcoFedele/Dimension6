...
diag = diagSimplify[diag] /. {q2-q3-q4 -> q234, q1^2 -> 0, q2^2 -> 0, q3^2 -> 0, q4^2 -> H};

prefactor = 1 / LAMBDA^2;

diag = Expand[diag / prefactor];
diag = diag /. {sp[Epa_, q_] :>  sp[q, Epa] /; MatchQ[Epa, Ep3] && ! MatchQ[q, Ep3]};
...
diag = ExpandScalarProducts[diag /. {q234^2 -> 0, q234 -> q2 - q3 - q4}];
diag = diag /. {B0[ - q1_ - q2_ , m1_ , m2_ ] -> B0[ q1 + q2 , m1 , m2 ], q1^2 -> 0, q2^2 -> 0, q3^2 -> 0, q4^2 -> h};
(*diag = diag /. {sp[Ep3, q1] -> 0, sp[Ep3, q2] -> 0, sp[Ep3, q4] -> 0} /. {sp[Epa_, q_] :>  sp[q, Epa] /; MatchQ[Epa, Ep3] && ! MatchQ[q, Ep3]};*)
...
Print[""];
total = Total[diag];
Table[Print["Entry relative to ", c, " : \n", Expand[ -2 Simplify[ Coefficient[total, c] / Sqrt[2] ] ],"\n"],{c,{cledq3322,cledq3333,cle2332,cle3333,cHl323,cHl333,clequ13322,clequ13333}}];


FILE = NotebookDirectory[]<>"tautauHW0ad.res";
DeleteFile[FILE]; OpenWrite[FILE];
WriteString[FILE,"\n"];WriteString[FILE,"\n"];
WriteString[FILE,"########  ANOMALOUS DIMENSION ENTRIES  ########"];
WriteString[FILE,"\n"];WriteString[FILE,"\n"];
Do[WriteString[FILE,ToString[c]<>"entry:=\n"];
   Write[FILE, Expand[ -2 Simplify[ Coefficient[total,c] / Sqrt[2] ] ] ];
   WriteString[FILE,"\n"];
