...
ftrans[exp_] := Module[
                       ...
                       temp = exp //.DEConvention;
                       
                       temp = temp /. {FourVector[-q3, Index[Lorentz, 1]]* (NonCommutative[ChiralityProjector[-1]] + NonCommutative[ChiralityProjector[1]]) -
                                        NonCommutative[DiracMatrix[Index[Lorentz, 1]], DiracSlash[-q3],ChiralityProjector[-1]] -
                                        NonCommutative[DiracMatrix[Index[Lorentz, 1]], DiracSlash[-q3], ChiralityProjector[1]] -> TAG1,
                                      FourVector[q3, Index[Lorentz, 1]]* (NonCommutative[ChiralityProjector[-1]] + NonCommutative[ChiralityProjector[1]]) +
                                        NonCommutative[DiracMatrix[Index[Lorentz, 1]], DiracSlash[-q3],ChiralityProjector[-1]] +
                                        NonCommutative[DiracMatrix[Index[Lorentz, 1]], DiracSlash[-q3], ChiralityProjector[1]] -> TAG2};
                       
                       temp = temp /. FermionChain[a_*TAG1, AA___] -> a*TAG1*MatrixTrace[AA] / (nd - 2);
                       temp = temp /. FermionChain[a_*TAG2, AA___] -> a*TAG2*MatrixTrace[AA] / (nd - 2);
                       temp = temp /. FermionChain[AA___, a_*TAG1] -> a*TAG1*MatrixTrace[AA] / (nd - 2);
                       temp = temp /. FermionChain[AA___, a_*TAG2] -> a*TAG2*MatrixTrace[AA] / (nd - 2);
                       
                      (* temp = Expand[ temp FourVector[Ep3, Index[Lorentz, 1]] ];*)
...
Paint[DiagramExtract[ins1, finaldiag], DisplayFunction -> (Export["tautauHW0_included.pdf",#]&), ColumnsXRows ->{3,4}];
Print["Included diagrams printed"];
(*If[Length[DiagramComplement[ins1, DiagramExtract[ins1, finaldiag]]] != 0,
   Paint[DiagramComplement[ins1, DiagramExtract[ins1, finaldiag]], DisplayFunction -> (Export["tautauHW0_excluded.pdf",#]&), ColumnsXRows ->{3,4}];
...
FILE = NotebookDirectory[]<>"tautauHW0diag.res";
