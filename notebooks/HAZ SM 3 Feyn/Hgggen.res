amp0=
FeynAmpList[Process -> {{V[1], FourMomentum[Incoming, 1], 0, {}}, 
     {V[2], FourMomentum[Incoming, 2], MZ, {}}} -> 
    {{S[1], FourMomentum[Outgoing, 1], MH, {}}}, Model -> {"SM"}, 
  GenericModel -> {"Lorentz"}, AmplitudeLevel -> {Particles}, 
  ExcludeParticles -> {F, -F[1], F[1], -F[2], F[2], -F[3], F[3], -F[4], F[4], 
    -F[1, {1}], F[1, {1}], -F[1, {2}], F[1, {2}], -F[1, {3}], F[1, {3}], 
    -F[2, {1}], F[2, {1}], -F[2, {2}], F[2, {2}], -F[2, {3}], F[2, {3}], 
    -F[3, {1, _}], F[3, {1, _}], -F[3, {2, _}], F[3, {2, _}], -F[3, {3, _}], 
    F[3, {3, _}], -F[4, {1, _}], F[4, {1, _}], -F[4, {2, _}], F[4, {2, _}], 
    -F[4, {3, _}], F[4, {3, _}], V[1]}, ExcludeFieldPoints -> {}, 
  LastSelections -> {}][FeynAmp[GraphID[Topology == 1, Generic == 1, 
   Particles == 1, Number == 1], Integral[FourMomentum[Internal, 1]], 
  -(EL^3*MH^2*(-1 + 2*s2)*FeynAmpDenominator[PropagatorDenominator[
       FourMomentum[Internal, 1], MW], PropagatorDenominator[
       FourMomentum[Incoming, 2] + FourMomentum[Internal, 1], MW], 
      PropagatorDenominator[FourMomentum[Incoming, 2] + 
        FourMomentum[Internal, 1] - FourMomentum[Outgoing, 1], MW]]*
     FourVector[FourMomentum[Incoming, 2] + 2*FourMomentum[Internal, 1], 
      Index[Lorentz, 2]]*FourVector[-FourMomentum[Incoming, 2] - 
       2*FourMomentum[Internal, 1] + FourMomentum[Outgoing, 1], 
      Index[Lorentz, 1]])/(4*MW*Sqrt[1 - s2]*s2)], 
 FeynAmp[GraphID[Topology == 1, Generic == 1, Particles == 2, Number == 2], 
  Integral[FourMomentum[Internal, 1]], 
  -(EL^3*MH^2*(-1 + 2*s2)*FeynAmpDenominator[PropagatorDenominator[
       FourMomentum[Internal, 1], MW], PropagatorDenominator[
       FourMomentum[Incoming, 2] + FourMomentum[Internal, 1], MW], 
      PropagatorDenominator[FourMomentum[Incoming, 2] + 
        FourMomentum[Internal, 1] - FourMomentum[Outgoing, 1], MW]]*
     FourVector[FourMomentum[Incoming, 2] + 2*FourMomentum[Internal, 1], 
      Index[Lorentz, 2]]*FourVector[-FourMomentum[Incoming, 2] - 
       2*FourMomentum[Internal, 1] + FourMomentum[Outgoing, 1], 
      Index[Lorentz, 1]])/(4*MW*Sqrt[1 - s2]*s2)], 
 FeynAmp[GraphID[Topology == 1, Generic == 2, Particles == 1, Number == 3], 
  Integral[FourMomentum[Internal, 1]], 
  (EL^3*MW*Sqrt[1 - s2]*FeynAmpDenominator[PropagatorDenominator[
      FourMomentum[Internal, 1], MW], PropagatorDenominator[
      FourMomentum[Incoming, 2] + FourMomentum[Internal, 1], MW], 
     PropagatorDenominator[FourMomentum[Incoming, 2] + 
       FourMomentum[Internal, 1] - FourMomentum[Outgoing, 1], MW]]*
    FourVector[FourMomentum[Internal, 1], Index[Lorentz, 2]]*
    FourVector[FourMomentum[Incoming, 2] + FourMomentum[Internal, 1] - 
      FourMomentum[Outgoing, 1], Index[Lorentz, 1]])/(2*s2)], 
 FeynAmp[GraphID[Topology == 1, Generic == 2, Particles == 2, Number == 4], 
  Integral[FourMomentum[Internal, 1]], 
  (EL^3*MW*Sqrt[1 - s2]*FeynAmpDenominator[PropagatorDenominator[
      FourMomentum[Internal, 1], MW], PropagatorDenominator[
      FourMomentum[Incoming, 2] + FourMomentum[Internal, 1], MW], 
     PropagatorDenominator[FourMomentum[Incoming, 2] + 
       FourMomentum[Internal, 1] - FourMomentum[Outgoing, 1], MW]]*
    FourVector[-FourMomentum[Incoming, 2] - FourMomentum[Internal, 1], 
     Index[Lorentz, 2]]*FourVector[-FourMomentum[Internal, 1], 
     Index[Lorentz, 1]])/(2*s2)], 
 FeynAmp[GraphID[Topology == 1, Generic == 2, Particles == 3, Number == 5], 
  Integral[FourMomentum[Internal, 1]], 
  (EL^3*MW*Sqrt[1 - s2]*FeynAmpDenominator[PropagatorDenominator[
      FourMomentum[Internal, 1], MW], PropagatorDenominator[
      FourMomentum[Incoming, 2] + FourMomentum[Internal, 1], MW], 
     PropagatorDenominator[FourMomentum[Incoming, 2] + 
       FourMomentum[Internal, 1] - FourMomentum[Outgoing, 1], MW]]*
    FourVector[FourMomentum[Internal, 1], Index[Lorentz, 2]]*
    FourVector[FourMomentum[Incoming, 2] + FourMomentum[Internal, 1] - 
      FourMomentum[Outgoing, 1], Index[Lorentz, 1]])/(2*s2)], 
 FeynAmp[GraphID[Topology == 1, Generic == 2, Particles == 4, Number == 6], 
  Integral[FourMomentum[Internal, 1]], 
  (EL^3*MW*Sqrt[1 - s2]*FeynAmpDenominator[PropagatorDenominator[
      FourMomentum[Internal, 1], MW], PropagatorDenominator[
      FourMomentum[Incoming, 2] + FourMomentum[Internal, 1], MW], 
     PropagatorDenominator[FourMomentum[Incoming, 2] + 
       FourMomentum[Internal, 1] - FourMomentum[Outgoing, 1], MW]]*
    FourVector[-FourMomentum[Incoming, 2] - FourMomentum[Internal, 1], 
     Index[Lorentz, 2]]*FourVector[-FourMomentum[Internal, 1], 
     Index[Lorentz, 1]])/(2*s2)], 
 FeynAmp[GraphID[Topology == 1, Generic == 3, Particles == 1, Number == 7], 
  Integral[FourMomentum[Internal, 1]], 
  (EL^3*MW*Sqrt[1 - s2]*FeynAmpDenominator[PropagatorDenominator[
      FourMomentum[Internal, 1], MW], PropagatorDenominator[
      FourMomentum[Incoming, 2] + FourMomentum[Internal, 1], MW], 
     PropagatorDenominator[FourMomentum[Incoming, 2] + 
       FourMomentum[Internal, 1] - FourMomentum[Outgoing, 1], MW]]*
    MetricTensor[Index[Lorentz, 3], Index[Lorentz, 4]]*
    (FourVector[-FourMomentum[Incoming, 1] - FourMomentum[Internal, 1], 
       Index[Lorentz, 5]]*MetricTensor[Index[Lorentz, 1], 
       Index[Lorentz, 3]] + FourVector[FourMomentum[Incoming, 1] - 
        FourMomentum[Incoming, 2] - FourMomentum[Internal, 1] + 
        FourMomentum[Outgoing, 1], Index[Lorentz, 3]]*
      MetricTensor[Index[Lorentz, 1], Index[Lorentz, 5]] + 
     FourVector[FourMomentum[Incoming, 2] + 2*FourMomentum[Internal, 1] - 
        FourMomentum[Outgoing, 1], Index[Lorentz, 1]]*
      MetricTensor[Index[Lorentz, 3], Index[Lorentz, 5]])*
    (FourVector[-FourMomentum[Incoming, 2] + FourMomentum[Internal, 1], 
       Index[Lorentz, 7]]*MetricTensor[Index[Lorentz, 2], 
       Index[Lorentz, 4]] + FourVector[2*FourMomentum[Incoming, 2] + 
        FourMomentum[Internal, 1], Index[Lorentz, 4]]*
      MetricTensor[Index[Lorentz, 2], Index[Lorentz, 7]] + 
     FourVector[-FourMomentum[Incoming, 2] - 2*FourMomentum[Internal, 1], 
       Index[Lorentz, 2]]*MetricTensor[Index[Lorentz, 4], Index[Lorentz, 7]])*
    MetricTensor[Index[Lorentz, 5], Index[Lorentz, 6]]*
    MetricTensor[Index[Lorentz, 6], Index[Lorentz, 8]]*
    MetricTensor[Index[Lorentz, 7], Index[Lorentz, 8]])/s2], 
 FeynAmp[GraphID[Topology == 1, Generic == 3, Particles == 2, Number == 8], 
  Integral[FourMomentum[Internal, 1]], 
  (EL^3*MW*Sqrt[1 - s2]*FeynAmpDenominator[PropagatorDenominator[
      FourMomentum[Internal, 1], MW], PropagatorDenominator[
      FourMomentum[Incoming, 2] + FourMomentum[Internal, 1], MW], 
     PropagatorDenominator[FourMomentum[Incoming, 2] + 
       FourMomentum[Internal, 1] - FourMomentum[Outgoing, 1], MW]]*
    MetricTensor[Index[Lorentz, 3], Index[Lorentz, 4]]*
    (FourVector[-FourMomentum[Incoming, 1] - FourMomentum[Internal, 1], 
       Index[Lorentz, 5]]*MetricTensor[Index[Lorentz, 1], 
       Index[Lorentz, 3]] + FourVector[FourMomentum[Incoming, 1] - 
        FourMomentum[Incoming, 2] - FourMomentum[Internal, 1] + 
        FourMomentum[Outgoing, 1], Index[Lorentz, 3]]*
      MetricTensor[Index[Lorentz, 1], Index[Lorentz, 5]] + 
     FourVector[FourMomentum[Incoming, 2] + 2*FourMomentum[Internal, 1] - 
        FourMomentum[Outgoing, 1], Index[Lorentz, 1]]*
      MetricTensor[Index[Lorentz, 3], Index[Lorentz, 5]])*
    (FourVector[-FourMomentum[Incoming, 2] + FourMomentum[Internal, 1], 
       Index[Lorentz, 7]]*MetricTensor[Index[Lorentz, 2], 
       Index[Lorentz, 4]] + FourVector[2*FourMomentum[Incoming, 2] + 
        FourMomentum[Internal, 1], Index[Lorentz, 4]]*
      MetricTensor[Index[Lorentz, 2], Index[Lorentz, 7]] + 
     FourVector[-FourMomentum[Incoming, 2] - 2*FourMomentum[Internal, 1], 
       Index[Lorentz, 2]]*MetricTensor[Index[Lorentz, 4], Index[Lorentz, 7]])*
    MetricTensor[Index[Lorentz, 5], Index[Lorentz, 6]]*
    MetricTensor[Index[Lorentz, 6], Index[Lorentz, 8]]*
    MetricTensor[Index[Lorentz, 7], Index[Lorentz, 8]])/s2], 
 FeynAmp[GraphID[Topology == 1, Generic == 4, Particles == 1, Number == 9], 
  Integral[FourMomentum[Internal, 1]], 
  -(EL^3*MW*FeynAmpDenominator[PropagatorDenominator[FourMomentum[Internal, 
        1], MW], PropagatorDenominator[FourMomentum[Incoming, 2] + 
        FourMomentum[Internal, 1], MW], PropagatorDenominator[
       FourMomentum[Incoming, 2] + FourMomentum[Internal, 1] - 
        FourMomentum[Outgoing, 1], MW]]*FourVector[
      FourMomentum[Incoming, 2] + FourMomentum[Internal, 1] - 
       2*FourMomentum[Outgoing, 1], Index[Lorentz, 4]]*
     FourVector[-FourMomentum[Incoming, 2] - 2*FourMomentum[Internal, 1] + 
       FourMomentum[Outgoing, 1], Index[Lorentz, 1]]*
     MetricTensor[Index[Lorentz, 2], Index[Lorentz, 3]]*
     MetricTensor[Index[Lorentz, 3], Index[Lorentz, 4]])/(2*Sqrt[1 - s2])], 
 FeynAmp[GraphID[Topology == 1, Generic == 4, Particles == 2, Number == 10], 
  Integral[FourMomentum[Internal, 1]], 
  -(EL^3*MW*FeynAmpDenominator[PropagatorDenominator[FourMomentum[Internal, 
        1], MW], PropagatorDenominator[FourMomentum[Incoming, 2] + 
        FourMomentum[Internal, 1], MW], PropagatorDenominator[
       FourMomentum[Incoming, 2] + FourMomentum[Internal, 1] - 
        FourMomentum[Outgoing, 1], MW]]*FourVector[
      FourMomentum[Incoming, 2] + FourMomentum[Internal, 1] - 
       2*FourMomentum[Outgoing, 1], Index[Lorentz, 4]]*
     FourVector[-FourMomentum[Incoming, 2] - 2*FourMomentum[Internal, 1] + 
       FourMomentum[Outgoing, 1], Index[Lorentz, 1]]*
     MetricTensor[Index[Lorentz, 2], Index[Lorentz, 3]]*
     MetricTensor[Index[Lorentz, 3], Index[Lorentz, 4]])/(2*Sqrt[1 - s2])], 
 FeynAmp[GraphID[Topology == 1, Generic == 5, Particles == 1, Number == 11], 
  Integral[FourMomentum[Internal, 1]], 
  -(EL^3*MW*(-1 + 2*s2)*FeynAmpDenominator[PropagatorDenominator[
       FourMomentum[Internal, 1], MW], PropagatorDenominator[
       FourMomentum[Incoming, 2] + FourMomentum[Internal, 1], MW], 
      PropagatorDenominator[FourMomentum[Incoming, 2] + 
        FourMomentum[Internal, 1] - FourMomentum[Outgoing, 1], MW]]*
     FourVector[FourMomentum[Incoming, 2] + 2*FourMomentum[Internal, 1], 
      Index[Lorentz, 2]]*FourVector[-FourMomentum[Incoming, 2] - 
       FourMomentum[Internal, 1] - FourMomentum[Outgoing, 1], 
      Index[Lorentz, 4]]*MetricTensor[Index[Lorentz, 1], Index[Lorentz, 3]]*
     MetricTensor[Index[Lorentz, 3], Index[Lorentz, 4]])/
   (4*Sqrt[1 - s2]*s2)], FeynAmp[GraphID[Topology == 1, Generic == 5, 
   Particles == 2, Number == 12], Integral[FourMomentum[Internal, 1]], 
  -(EL^3*MW*(-1 + 2*s2)*FeynAmpDenominator[PropagatorDenominator[
       FourMomentum[Internal, 1], MW], PropagatorDenominator[
       FourMomentum[Incoming, 2] + FourMomentum[Internal, 1], MW], 
      PropagatorDenominator[FourMomentum[Incoming, 2] + 
        FourMomentum[Internal, 1] - FourMomentum[Outgoing, 1], MW]]*
     FourVector[FourMomentum[Incoming, 2] + 2*FourMomentum[Internal, 1], 
      Index[Lorentz, 2]]*FourVector[-FourMomentum[Incoming, 2] - 
       FourMomentum[Internal, 1] - FourMomentum[Outgoing, 1], 
      Index[Lorentz, 4]]*MetricTensor[Index[Lorentz, 1], Index[Lorentz, 3]]*
     MetricTensor[Index[Lorentz, 3], Index[Lorentz, 4]])/
   (4*Sqrt[1 - s2]*s2)], FeynAmp[GraphID[Topology == 1, Generic == 6, 
   Particles == 1, Number == 13], Integral[FourMomentum[Internal, 1]], 
  -(EL^3*MH^2*MW*FeynAmpDenominator[PropagatorDenominator[
       FourMomentum[Internal, 1], MW], PropagatorDenominator[
       FourMomentum[Incoming, 2] + FourMomentum[Internal, 1], MW], 
      PropagatorDenominator[FourMomentum[Incoming, 2] + 
        FourMomentum[Internal, 1] - FourMomentum[Outgoing, 1], MW]]*
     MetricTensor[Index[Lorentz, 1], Index[Lorentz, 3]]*
     MetricTensor[Index[Lorentz, 2], Index[Lorentz, 4]]*
     MetricTensor[Index[Lorentz, 3], Index[Lorentz, 4]])/(2*Sqrt[1 - s2])], 
 FeynAmp[GraphID[Topology == 1, Generic == 6, Particles == 2, Number == 14], 
  Integral[FourMomentum[Internal, 1]], 
  -(EL^3*MH^2*MW*FeynAmpDenominator[PropagatorDenominator[
       FourMomentum[Internal, 1], MW], PropagatorDenominator[
       FourMomentum[Incoming, 2] + FourMomentum[Internal, 1], MW], 
      PropagatorDenominator[FourMomentum[Incoming, 2] + 
        FourMomentum[Internal, 1] - FourMomentum[Outgoing, 1], MW]]*
     MetricTensor[Index[Lorentz, 1], Index[Lorentz, 3]]*
     MetricTensor[Index[Lorentz, 2], Index[Lorentz, 4]]*
     MetricTensor[Index[Lorentz, 3], Index[Lorentz, 4]])/(2*Sqrt[1 - s2])], 
 FeynAmp[GraphID[Topology == 1, Generic == 7, Particles == 1, Number == 15], 
  Integral[FourMomentum[Internal, 1]], 
  -((EL^3*MW^3*FeynAmpDenominator[PropagatorDenominator[
       FourMomentum[Internal, 1], MW], PropagatorDenominator[
       FourMomentum[Incoming, 2] + FourMomentum[Internal, 1], MW], 
      PropagatorDenominator[FourMomentum[Incoming, 2] + 
        FourMomentum[Internal, 1] - FourMomentum[Outgoing, 1], MW]]*
     MetricTensor[Index[Lorentz, 1], Index[Lorentz, 3]]*
     MetricTensor[Index[Lorentz, 2], Index[Lorentz, 5]]*
     MetricTensor[Index[Lorentz, 3], Index[Lorentz, 4]]*
     MetricTensor[Index[Lorentz, 4], Index[Lorentz, 6]]*
     MetricTensor[Index[Lorentz, 5], Index[Lorentz, 6]])/Sqrt[1 - s2])], 
 FeynAmp[GraphID[Topology == 1, Generic == 7, Particles == 2, Number == 16], 
  Integral[FourMomentum[Internal, 1]], 
  -((EL^3*MW^3*FeynAmpDenominator[PropagatorDenominator[
       FourMomentum[Internal, 1], MW], PropagatorDenominator[
       FourMomentum[Incoming, 2] + FourMomentum[Internal, 1], MW], 
      PropagatorDenominator[FourMomentum[Incoming, 2] + 
        FourMomentum[Internal, 1] - FourMomentum[Outgoing, 1], MW]]*
     MetricTensor[Index[Lorentz, 1], Index[Lorentz, 3]]*
     MetricTensor[Index[Lorentz, 2], Index[Lorentz, 5]]*
     MetricTensor[Index[Lorentz, 3], Index[Lorentz, 4]]*
     MetricTensor[Index[Lorentz, 4], Index[Lorentz, 6]]*
     MetricTensor[Index[Lorentz, 5], Index[Lorentz, 6]])/Sqrt[1 - s2])], 
 FeynAmp[GraphID[Topology == 1, Generic == 8, Particles == 1, Number == 17], 
  Integral[FourMomentum[Internal, 1]], 
  (EL^3*MW*Sqrt[1 - s2]*FeynAmpDenominator[PropagatorDenominator[
      FourMomentum[Internal, 1], MW], PropagatorDenominator[
      FourMomentum[Incoming, 2] + FourMomentum[Internal, 1], MW], 
     PropagatorDenominator[FourMomentum[Incoming, 2] + 
       FourMomentum[Internal, 1] - FourMomentum[Outgoing, 1], MW]]*
    FourVector[FourMomentum[Incoming, 2] + FourMomentum[Internal, 1] - 
      2*FourMomentum[Outgoing, 1], Index[Lorentz, 6]]*
    MetricTensor[Index[Lorentz, 1], Index[Lorentz, 3]]*
    MetricTensor[Index[Lorentz, 3], Index[Lorentz, 4]]*
    (FourVector[-FourMomentum[Incoming, 2] + FourMomentum[Internal, 1], 
       Index[Lorentz, 5]]*MetricTensor[Index[Lorentz, 2], 
       Index[Lorentz, 4]] + FourVector[2*FourMomentum[Incoming, 2] + 
        FourMomentum[Internal, 1], Index[Lorentz, 4]]*
      MetricTensor[Index[Lorentz, 2], Index[Lorentz, 5]] + 
     FourVector[-FourMomentum[Incoming, 2] - 2*FourMomentum[Internal, 1], 
       Index[Lorentz, 2]]*MetricTensor[Index[Lorentz, 4], Index[Lorentz, 5]])*
    MetricTensor[Index[Lorentz, 5], Index[Lorentz, 6]])/(2*s2)], 
 FeynAmp[GraphID[Topology == 1, Generic == 8, Particles == 2, Number == 18], 
  Integral[FourMomentum[Internal, 1]], 
  (EL^3*MW*Sqrt[1 - s2]*FeynAmpDenominator[PropagatorDenominator[
      FourMomentum[Internal, 1], MW], PropagatorDenominator[
      FourMomentum[Incoming, 2] + FourMomentum[Internal, 1], MW], 
     PropagatorDenominator[FourMomentum[Incoming, 2] + 
       FourMomentum[Internal, 1] - FourMomentum[Outgoing, 1], MW]]*
    FourVector[FourMomentum[Incoming, 2] + FourMomentum[Internal, 1] - 
      2*FourMomentum[Outgoing, 1], Index[Lorentz, 6]]*
    MetricTensor[Index[Lorentz, 1], Index[Lorentz, 3]]*
    MetricTensor[Index[Lorentz, 3], Index[Lorentz, 4]]*
    (FourVector[-FourMomentum[Incoming, 2] + FourMomentum[Internal, 1], 
       Index[Lorentz, 5]]*MetricTensor[Index[Lorentz, 2], 
       Index[Lorentz, 4]] + FourVector[2*FourMomentum[Incoming, 2] + 
        FourMomentum[Internal, 1], Index[Lorentz, 4]]*
      MetricTensor[Index[Lorentz, 2], Index[Lorentz, 5]] + 
     FourVector[-FourMomentum[Incoming, 2] - 2*FourMomentum[Internal, 1], 
       Index[Lorentz, 2]]*MetricTensor[Index[Lorentz, 4], Index[Lorentz, 5]])*
    MetricTensor[Index[Lorentz, 5], Index[Lorentz, 6]])/(2*s2)], 
 FeynAmp[GraphID[Topology == 1, Generic == 9, Particles == 1, Number == 19], 
  Integral[FourMomentum[Internal, 1]], 
  -(EL^3*MW*FeynAmpDenominator[PropagatorDenominator[FourMomentum[Internal, 
        1], MW], PropagatorDenominator[FourMomentum[Incoming, 2] + 
        FourMomentum[Internal, 1], MW], PropagatorDenominator[
       FourMomentum[Incoming, 2] + FourMomentum[Internal, 1] - 
        FourMomentum[Outgoing, 1], MW]]*FourVector[
      -FourMomentum[Incoming, 2] - FourMomentum[Internal, 1] - 
       FourMomentum[Outgoing, 1], Index[Lorentz, 6]]*
     MetricTensor[Index[Lorentz, 2], Index[Lorentz, 4]]*
     MetricTensor[Index[Lorentz, 3], Index[Lorentz, 4]]*
     (FourVector[-FourMomentum[Incoming, 1] - FourMomentum[Internal, 1], 
        Index[Lorentz, 5]]*MetricTensor[Index[Lorentz, 1], 
        Index[Lorentz, 3]] + FourVector[FourMomentum[Incoming, 1] - 
         FourMomentum[Incoming, 2] - FourMomentum[Internal, 1] + 
         FourMomentum[Outgoing, 1], Index[Lorentz, 3]]*
       MetricTensor[Index[Lorentz, 1], Index[Lorentz, 5]] + 
      FourVector[FourMomentum[Incoming, 2] + 2*FourMomentum[Internal, 1] - 
         FourMomentum[Outgoing, 1], Index[Lorentz, 1]]*
       MetricTensor[Index[Lorentz, 3], Index[Lorentz, 5]])*
     MetricTensor[Index[Lorentz, 5], Index[Lorentz, 6]])/(2*Sqrt[1 - s2])], 
 FeynAmp[GraphID[Topology == 1, Generic == 9, Particles == 2, Number == 20], 
  Integral[FourMomentum[Internal, 1]], 
  -(EL^3*MW*FeynAmpDenominator[PropagatorDenominator[FourMomentum[Internal, 
        1], MW], PropagatorDenominator[FourMomentum[Incoming, 2] + 
        FourMomentum[Internal, 1], MW], PropagatorDenominator[
       FourMomentum[Incoming, 2] + FourMomentum[Internal, 1] - 
        FourMomentum[Outgoing, 1], MW]]*FourVector[
      -FourMomentum[Incoming, 2] - FourMomentum[Internal, 1] - 
       FourMomentum[Outgoing, 1], Index[Lorentz, 6]]*
     MetricTensor[Index[Lorentz, 2], Index[Lorentz, 4]]*
     MetricTensor[Index[Lorentz, 3], Index[Lorentz, 4]]*
     (FourVector[-FourMomentum[Incoming, 1] - FourMomentum[Internal, 1], 
        Index[Lorentz, 5]]*MetricTensor[Index[Lorentz, 1], 
        Index[Lorentz, 3]] + FourVector[FourMomentum[Incoming, 1] - 
         FourMomentum[Incoming, 2] - FourMomentum[Internal, 1] + 
         FourMomentum[Outgoing, 1], Index[Lorentz, 3]]*
       MetricTensor[Index[Lorentz, 1], Index[Lorentz, 5]] + 
      FourVector[FourMomentum[Incoming, 2] + 2*FourMomentum[Internal, 1] - 
         FourMomentum[Outgoing, 1], Index[Lorentz, 1]]*
       MetricTensor[Index[Lorentz, 3], Index[Lorentz, 5]])*
     MetricTensor[Index[Lorentz, 5], Index[Lorentz, 6]])/(2*Sqrt[1 - s2])], 
 FeynAmp[GraphID[Topology == 2, Generic == 1, Particles == 1, Number == 21], 
  Integral[FourMomentum[Internal, 1]], 
  -(EL^3*MW*FeynAmpDenominator[PropagatorDenominator[FourMomentum[Internal, 
        1], MW], PropagatorDenominator[FourMomentum[Incoming, 2] + 
        FourMomentum[Internal, 1] - FourMomentum[Outgoing, 1], MW]]*
     MetricTensor[Index[Lorentz, 1], Index[Lorentz, 3]]*
     MetricTensor[Index[Lorentz, 2], Index[Lorentz, 4]]*
     MetricTensor[Index[Lorentz, 3], Index[Lorentz, 4]])/(2*Sqrt[1 - s2])], 
 FeynAmp[GraphID[Topology == 2, Generic == 1, Particles == 2, Number == 22], 
  Integral[FourMomentum[Internal, 1]], 
  -(EL^3*MW*FeynAmpDenominator[PropagatorDenominator[FourMomentum[Internal, 
        1], MW], PropagatorDenominator[FourMomentum[Incoming, 2] + 
        FourMomentum[Internal, 1] - FourMomentum[Outgoing, 1], MW]]*
     MetricTensor[Index[Lorentz, 1], Index[Lorentz, 3]]*
     MetricTensor[Index[Lorentz, 2], Index[Lorentz, 4]]*
     MetricTensor[Index[Lorentz, 3], Index[Lorentz, 4]])/(2*Sqrt[1 - s2])], 
 FeynAmp[GraphID[Topology == 3, Generic == 1, Particles == 1, Number == 23], 
  Integral[FourMomentum[Internal, 1]], 
  -(EL^3*MW*FeynAmpDenominator[PropagatorDenominator[FourMomentum[Internal, 
        1], MW], PropagatorDenominator[-FourMomentum[Incoming, 2] + 
        FourMomentum[Internal, 1], MW]]*MetricTensor[Index[Lorentz, 1], 
      Index[Lorentz, 4]]*MetricTensor[Index[Lorentz, 2], Index[Lorentz, 3]]*
     MetricTensor[Index[Lorentz, 3], Index[Lorentz, 4]])/(2*Sqrt[1 - s2])], 
 FeynAmp[GraphID[Topology == 3, Generic == 1, Particles == 2, Number == 24], 
  Integral[FourMomentum[Internal, 1]], 
  -(EL^3*MW*FeynAmpDenominator[PropagatorDenominator[FourMomentum[Internal, 
        1], MW], PropagatorDenominator[-FourMomentum[Incoming, 2] + 
        FourMomentum[Internal, 1], MW]]*MetricTensor[Index[Lorentz, 1], 
      Index[Lorentz, 4]]*MetricTensor[Index[Lorentz, 2], Index[Lorentz, 3]]*
     MetricTensor[Index[Lorentz, 3], Index[Lorentz, 4]])/(2*Sqrt[1 - s2])], 
 FeynAmp[GraphID[Topology == 4, Generic == 1, Particles == 1, Number == 25], 
  Integral[FourMomentum[Internal, 1]], 
  -(EL^3*MH^2*(-1 + 2*s2)*FeynAmpDenominator[PropagatorDenominator[
       FourMomentum[Internal, 1], MW], PropagatorDenominator[
       FourMomentum[Internal, 1] + FourMomentum[Outgoing, 1], MW]]*
     MetricTensor[Index[Lorentz, 1], Index[Lorentz, 2]])/
   (2*MW*Sqrt[1 - s2]*s2)], FeynAmp[GraphID[Topology == 4, Generic == 2, 
   Particles == 1, Number == 26], Integral[FourMomentum[Internal, 1]], 
  ((-I)*EL*MW*FeynAmpDenominator[PropagatorDenominator[
      FourMomentum[Internal, 1], MW], PropagatorDenominator[
      FourMomentum[Internal, 1] + FourMomentum[Outgoing, 1], MW]]*
    MetricTensor[Index[Lorentz, 3], Index[Lorentz, 4]]*
    MetricTensor[Index[Lorentz, 3], Index[Lorentz, 5]]*
    (((-I)*EL^2*Sqrt[1 - s2]*MetricTensor[Index[Lorentz, 1], 
        Index[Lorentz, 6]]*MetricTensor[Index[Lorentz, 2], 
        Index[Lorentz, 4]])/Sqrt[s2] - 
     (I*EL^2*Sqrt[1 - s2]*MetricTensor[Index[Lorentz, 1], Index[Lorentz, 4]]*
       MetricTensor[Index[Lorentz, 2], Index[Lorentz, 6]])/Sqrt[s2] + 
     ((2*I)*EL^2*Sqrt[1 - s2]*MetricTensor[Index[Lorentz, 1], 
        Index[Lorentz, 2]]*MetricTensor[Index[Lorentz, 4], 
        Index[Lorentz, 6]])/Sqrt[s2])*MetricTensor[Index[Lorentz, 5], 
     Index[Lorentz, 6]])/Sqrt[s2]]]
ins1=
TopologyList[Process -> {V[1], V[2]} -> {S[1]}, Model -> {"SM"}, 
  GenericModel -> {"Lorentz"}, InsertionLevel -> {Particles}, 
  ExcludeParticles -> {F, -F[1], F[1], -F[2], F[2], -F[3], F[3], -F[4], F[4], 
    -F[1, {1}], F[1, {1}], -F[1, {2}], F[1, {2}], -F[1, {3}], F[1, {3}], 
    -F[2, {1}], F[2, {1}], -F[2, {2}], F[2, {2}], -F[2, {3}], F[2, {3}], 
    -F[3, {1, _}], F[3, {1, _}], -F[3, {2, _}], F[3, {2, _}], -F[3, {3, _}], 
    F[3, {3, _}], -F[4, {1, _}], F[4, {1, _}], -F[4, {2, _}], F[4, {2, _}], 
    -F[4, {3, _}], F[4, {3, _}], V[1]}, ExcludeFieldPoints -> {}, 
  LastSelections -> {}][Topology[1][Propagator[Incoming][Vertex[1][1], 
    Vertex[3][4], Field[1]], Propagator[Incoming][Vertex[1][2], Vertex[3][5], 
    Field[2]], Propagator[Outgoing][Vertex[1][3], Vertex[3][6], Field[3]], 
   Propagator[Loop[1]][Vertex[3][4], Vertex[3][5], Field[4]], 
   Propagator[Loop[1]][Vertex[3][4], Vertex[3][6], Field[5]], 
   Propagator[Loop[1]][Vertex[3][5], Vertex[3][6], Field[6]]] -> 
  Insertions[Generic][FeynmanGraph[1, Generic == 1][Field[1] -> V[1], 
     Field[2] -> V[2], Field[3] -> S[1], Field[4] -> S, Field[5] -> S, 
     Field[6] -> S] -> Insertions[Particles][FeynmanGraph[1, Particles == 1][
      Field[1] -> V[1], Field[2] -> V[2], Field[3] -> S[1], 
      Field[4] -> -S[3], Field[5] -> S[3], Field[6] -> -S[3]], 
     FeynmanGraph[1, Particles == 2][Field[1] -> V[1], Field[2] -> V[2], 
      Field[3] -> S[1], Field[4] -> S[3], Field[5] -> -S[3], 
      Field[6] -> S[3]]], FeynmanGraph[1, Generic == 2][Field[1] -> V[1], 
     Field[2] -> V[2], Field[3] -> S[1], Field[4] -> U, Field[5] -> U, 
     Field[6] -> U] -> Insertions[Particles][FeynmanGraph[1, Particles == 1][
      Field[1] -> V[1], Field[2] -> V[2], Field[3] -> S[1], 
      Field[4] -> -U[3], Field[5] -> U[3], Field[6] -> -U[3]], 
     FeynmanGraph[1, Particles == 2][Field[1] -> V[1], Field[2] -> V[2], 
      Field[3] -> S[1], Field[4] -> U[3], Field[5] -> -U[3], 
      Field[6] -> U[3]], FeynmanGraph[1, Particles == 3][Field[1] -> V[1], 
      Field[2] -> V[2], Field[3] -> S[1], Field[4] -> -U[4], 
      Field[5] -> U[4], Field[6] -> -U[4]], FeynmanGraph[1, Particles == 4][
      Field[1] -> V[1], Field[2] -> V[2], Field[3] -> S[1], Field[4] -> U[4], 
      Field[5] -> -U[4], Field[6] -> U[4]]], 
   FeynmanGraph[1, Generic == 3][Field[1] -> V[1], Field[2] -> V[2], 
     Field[3] -> S[1], Field[4] -> V, Field[5] -> V, Field[6] -> V] -> 
    Insertions[Particles][FeynmanGraph[1, Particles == 1][Field[1] -> V[1], 
      Field[2] -> V[2], Field[3] -> S[1], Field[4] -> -V[3], 
      Field[5] -> V[3], Field[6] -> -V[3]], FeynmanGraph[1, Particles == 2][
      Field[1] -> V[1], Field[2] -> V[2], Field[3] -> S[1], Field[4] -> V[3], 
      Field[5] -> -V[3], Field[6] -> V[3]]], 
   FeynmanGraph[1, Generic == 4][Field[1] -> V[1], Field[2] -> V[2], 
     Field[3] -> S[1], Field[4] -> S, Field[5] -> S, Field[6] -> V] -> 
    Insertions[Particles][FeynmanGraph[1, Particles == 1][Field[1] -> V[1], 
      Field[2] -> V[2], Field[3] -> S[1], Field[4] -> -S[3], 
      Field[5] -> S[3], Field[6] -> -V[3]], FeynmanGraph[1, Particles == 2][
      Field[1] -> V[1], Field[2] -> V[2], Field[3] -> S[1], Field[4] -> S[3], 
      Field[5] -> -S[3], Field[6] -> V[3]]], 
   FeynmanGraph[1, Generic == 5][Field[1] -> V[1], Field[2] -> V[2], 
     Field[3] -> S[1], Field[4] -> S, Field[5] -> V, Field[6] -> S] -> 
    Insertions[Particles][FeynmanGraph[1, Particles == 1][Field[1] -> V[1], 
      Field[2] -> V[2], Field[3] -> S[1], Field[4] -> -S[3], 
      Field[5] -> V[3], Field[6] -> -S[3]], FeynmanGraph[1, Particles == 2][
      Field[1] -> V[1], Field[2] -> V[2], Field[3] -> S[1], Field[4] -> S[3], 
      Field[5] -> -V[3], Field[6] -> S[3]]], 
   FeynmanGraph[1, Generic == 6][Field[1] -> V[1], Field[2] -> V[2], 
     Field[3] -> S[1], Field[4] -> V, Field[5] -> S, Field[6] -> S] -> 
    Insertions[Particles][FeynmanGraph[1, Particles == 1][Field[1] -> V[1], 
      Field[2] -> V[2], Field[3] -> S[1], Field[4] -> -V[3], 
      Field[5] -> S[3], Field[6] -> -S[3]], FeynmanGraph[1, Particles == 2][
      Field[1] -> V[1], Field[2] -> V[2], Field[3] -> S[1], Field[4] -> V[3], 
      Field[5] -> -S[3], Field[6] -> S[3]]], 
   FeynmanGraph[1, Generic == 7][Field[1] -> V[1], Field[2] -> V[2], 
     Field[3] -> S[1], Field[4] -> S, Field[5] -> V, Field[6] -> V] -> 
    Insertions[Particles][FeynmanGraph[1, Particles == 1][Field[1] -> V[1], 
      Field[2] -> V[2], Field[3] -> S[1], Field[4] -> -S[3], 
      Field[5] -> V[3], Field[6] -> -V[3]], FeynmanGraph[1, Particles == 2][
      Field[1] -> V[1], Field[2] -> V[2], Field[3] -> S[1], Field[4] -> S[3], 
      Field[5] -> -V[3], Field[6] -> V[3]]], 
   FeynmanGraph[1, Generic == 8][Field[1] -> V[1], Field[2] -> V[2], 
     Field[3] -> S[1], Field[4] -> V, Field[5] -> S, Field[6] -> V] -> 
    Insertions[Particles][FeynmanGraph[1, Particles == 1][Field[1] -> V[1], 
      Field[2] -> V[2], Field[3] -> S[1], Field[4] -> -V[3], 
      Field[5] -> S[3], Field[6] -> -V[3]], FeynmanGraph[1, Particles == 2][
      Field[1] -> V[1], Field[2] -> V[2], Field[3] -> S[1], Field[4] -> V[3], 
      Field[5] -> -S[3], Field[6] -> V[3]]], 
   FeynmanGraph[1, Generic == 9][Field[1] -> V[1], Field[2] -> V[2], 
     Field[3] -> S[1], Field[4] -> V, Field[5] -> V, Field[6] -> S] -> 
    Insertions[Particles][FeynmanGraph[1, Particles == 1][Field[1] -> V[1], 
      Field[2] -> V[2], Field[3] -> S[1], Field[4] -> -V[3], 
      Field[5] -> V[3], Field[6] -> -S[3]], FeynmanGraph[1, Particles == 2][
      Field[1] -> V[1], Field[2] -> V[2], Field[3] -> S[1], Field[4] -> V[3], 
      Field[5] -> -V[3], Field[6] -> S[3]]]], 
 Topology[2][Propagator[Incoming][Vertex[1][1], Vertex[3][4], Field[1]], 
   Propagator[Incoming][Vertex[1][2], Vertex[4][5], Field[2]], 
   Propagator[Outgoing][Vertex[1][3], Vertex[4][5], Field[3]], 
   Propagator[Loop[1]][Vertex[3][4], Vertex[4][5], Field[4]], 
   Propagator[Loop[1]][Vertex[3][4], Vertex[4][5], Field[5]]] -> 
  Insertions[Generic][FeynmanGraph[1, Generic == 1][Field[1] -> V[1], 
     Field[2] -> V[2], Field[3] -> S[1], Field[4] -> S, Field[5] -> V] -> 
    Insertions[Particles][FeynmanGraph[1, Particles == 1][Field[1] -> V[1], 
      Field[2] -> V[2], Field[3] -> S[1], Field[4] -> -S[3], 
      Field[5] -> V[3]], FeynmanGraph[1, Particles == 2][Field[1] -> V[1], 
      Field[2] -> V[2], Field[3] -> S[1], Field[4] -> S[3], 
      Field[5] -> -V[3]]]], 
 Topology[2][Propagator[Incoming][Vertex[1][1], Vertex[4][4], Field[1]], 
   Propagator[Incoming][Vertex[1][2], Vertex[3][5], Field[2]], 
   Propagator[Outgoing][Vertex[1][3], Vertex[4][4], Field[3]], 
   Propagator[Loop[1]][Vertex[3][5], Vertex[4][4], Field[4]], 
   Propagator[Loop[1]][Vertex[3][5], Vertex[4][4], Field[5]]] -> 
  Insertions[Generic][FeynmanGraph[1, Generic == 1][Field[1] -> V[1], 
     Field[2] -> V[2], Field[3] -> S[1], Field[4] -> S, Field[5] -> V] -> 
    Insertions[Particles][FeynmanGraph[1, Particles == 1][Field[1] -> V[1], 
      Field[2] -> V[2], Field[3] -> S[1], Field[4] -> -S[3], 
      Field[5] -> V[3]], FeynmanGraph[1, Particles == 2][Field[1] -> V[1], 
      Field[2] -> V[2], Field[3] -> S[1], Field[4] -> S[3], 
      Field[5] -> -V[3]]]], 
 Topology[2][Propagator[Incoming][Vertex[1][1], Vertex[4][4], Field[1]], 
   Propagator[Incoming][Vertex[1][2], Vertex[4][4], Field[2]], 
   Propagator[Outgoing][Vertex[1][3], Vertex[3][5], Field[3]], 
   Propagator[Loop[1]][Vertex[3][5], Vertex[4][4], Field[4]], 
   Propagator[Loop[1]][Vertex[3][5], Vertex[4][4], Field[5]]] -> 
  Insertions[Generic][FeynmanGraph[2, Generic == 1][Field[1] -> V[1], 
     Field[2] -> V[2], Field[3] -> S[1], Field[4] -> S, Field[5] -> S] -> 
    Insertions[Particles][FeynmanGraph[1, Particles == 1][Field[1] -> V[1], 
      Field[2] -> V[2], Field[3] -> S[1], Field[4] -> -S[3], 
      Field[5] -> S[3]]], FeynmanGraph[2, Generic == 2][Field[1] -> V[1], 
     Field[2] -> V[2], Field[3] -> S[1], Field[4] -> V, Field[5] -> V] -> 
    Insertions[Particles][FeynmanGraph[1, Particles == 1][Field[1] -> V[1], 
      Field[2] -> V[2], Field[3] -> S[1], Field[4] -> -V[3], 
      Field[5] -> V[3]]]]]
