...
diag = ExpandScalarProducts[diag /. {q123 -> q1 + q2 - q3}];
diag = diag //. {q1 q2 -> sp[q1, q2], q2 q3 -> sp[q2, q3], q1 q3 -> sp[q1, q3]};
...
diag = Simplify[diag];
diag = spReabs[diag];
diag = diag /. sp[q1 + q2 - q3, q1 + q2 - q3] -> h;
(*Print[""];
...
Print[""];
total = Total[diag];
Table[Print["Entry relative to ", c, " : \n", FullSimplify[ Expand[ -2 Simplify[ Coefficient[total, c] * (-1/6) ] ] ],"\n"],{c,{cuH33,cdH33,ceH33}}];
...
Do[WriteString[FILE,ToString[c]<>"entry:=\n"];
   Write[FILE, FullSimplify[ Expand[ -2 Simplify[ Coefficient[total, c] * (-1/6) ] ] ] ];
   WriteString[FILE,"\n"];
   ,{c,{cuH33,cdH33,ceH33}}];
WriteString[FILE,"\n"];WriteString[FILE,"\n"];
