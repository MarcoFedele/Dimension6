...
If[!MatchQ[Coefficient[totale, AAAA, 0], 0] ,
   ...
   extraad = Simplify[Coefficient[totale, AAAA, 0]];
   Print[""];
...
If[!MatchQ[Coefficient[total0, AAAA], 0] ,
   ...
   ft = Collect[ Coefficient[total0/2, AAAA] /. {w -> gw^2 vev^2/4, h -> 2 lam vev^2}, {e, c\[Gamma]\[Gamma], cW, cWB,
    Log[_], A0[_], B0[___], C0[___], vev, w}, Simplify]//. {gw^2 vev^2/4 -> w, 2 lam vev^2 -> h};
...
If[!MatchQ[Coefficient[total0, AAAA, 0], 0] ,
   ...
   extraft = Collect[ Coefficient[total0, AAAA, 0] /. {w -> gw^2 vev^2/4, h -> 2 lam vev^2}, {sp[Ep1, Ep2], e, c\[Gamma]\[Gamma], cW, cWB, Log[_], A0[_], B0[___], C0[___], vev, w}, Simplify]//. {gw^2 vev^2/4 -> w, 2 lam vev^2 -> h};
   Print[""];
   Print["There's something else!"]
   Print[""];
   Print[ extraft ];
