...
process1 = {S[1], S[1], S[1]} -> {S[1], S[1], S[1]};

top = CreateTopologies[1, 3 -> 3, Adjacencies -> {3, 4, 5}, ExcludeTopologies -> {Tadpoles, SelfEnergies, Triangles, Pentagons, Hexagons} ];

 ins1 = InsertFields[ top, process1, Model -> "Mine/1308.2627.4f", GenericModel -> "Mine/1308.2627.4f", ExcludeParticles -> {V, F[1|2|3|4|5|7|8|10|11], S[2|3]}, InsertionLevel->{Particles} ];
 ...
 amp0 = amp0 /. {MatrixTrace[x___, cuH33/LAMBDA^2*a_*NonCommutative[ChiralityProjector[-1]] + cuH33/LAMBDA^2*a_*NonCommutative[ChiralityProjector[1]], y___] ->
                    cuH33/LAMBDA^2*MatrixTrace[x, a NonCommutative[ChiralityProjector[-1]] + a NonCommutative[ChiralityProjector[1]], y],
                 MatrixTrace[x___, cdH33/LAMBDA^2*a_*NonCommutative[ChiralityProjector[-1]] + cdH33/LAMBDA^2*a_*NonCommutative[ChiralityProjector[1]], y___] ->
                    cdH33/LAMBDA^2*MatrixTrace[x, a NonCommutative[ChiralityProjector[-1]] + a NonCommutative[ChiralityProjector[1]], y],
                 MatrixTrace[x___, ceH33/LAMBDA^2*a_*NonCommutative[ChiralityProjector[-1]] + ceH33/LAMBDA^2*a_*NonCommutative[ChiralityProjector[1]], y___] ->
                    ceH33/LAMBDA^2*MatrixTrace[x, a NonCommutative[ChiralityProjector[-1]] + a NonCommutative[ChiralityProjector[1]], y]};
