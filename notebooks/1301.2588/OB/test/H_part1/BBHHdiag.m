...
Paint[DiagramExtract[ins1, finaldiag], DisplayFunction -> (Export["BBHH_included.pdf",#]&), ColumnsXRows ->{3,4}];
Print["Included diagrams printed"];
If[Length[DiagramComplement[ins1, DiagramExtract[ins1, finaldiag]]] != 0,
   Paint[DiagramComplement[ins1, DiagramExtract[ins1, finaldiag]], DisplayFunction -> (Export["BBHH_excluded.pdf",#]&), ColumnsXRows ->{3,4}];
...
Paint[DiagramExtract[DiagramExtract[ins1, finaldiag], Table[i,{i,21,31}] ] , DisplayFunction -> (Export["WBHH_prova.pdf",#]&), ColumnsXRows ->{3,4}];
temp = {0};
Do[temp = Append[temp, amp2[[i]]], {i, Table[j, {j,21,31}]}];
amp2 = Rest[temp];


FILE = NotebookDirectory[]<>"BBHHdiag.res";
