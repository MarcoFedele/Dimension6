...
PVsub := {pp[p, m1_] pp[p + x_, m2_] pp[p + y_, m3_] -> C0[x, y, m1, m2, m3],
         ...
         pp[p, m1_] pp[p - x_, m2_] -> 1/e + B0[x^2, m1, m2] + e B1[x^2,m1,m2],
         pp[p + (a_Integer:1) x_, m1_] pp[p + (b_Integer:1) y_, m2_] -> 1/e + B0[-a x^2 + b y^2, m1, m2] + e B1[-a x^2 + b y^2,m1,m2],
         pp[___, m_] -> m/e + A0[m] + e A1[m]};
...
diag = diag /. nd -> 4 - 2e;
diag = Normal[Series[diag, {e, 0, 1}]];

diag = diag /. { A0[a_] -> -a Log[a] + a,
                 A1[a_] -> a Pi^2/12 + (2 a - 2 a Log[a] + a Log[a]^2)/2} ;
diag = diag /. { B0[a_,0,0] -> -Log[a] +2,
                 B1[a_,0,0] -> -7Pi^2/12 + (8 - 4*Log[a] + Log[a]^2)/2} ;
diag = diag /. { B0[a_,b_,0] -> B0[a,0,b], B1[a_,b_,0] -> B1[a,0,b]} ;
diag = diag /. { B0[a_,0,a_] -> -Log[a] +2,
                 B1[a_,0,a_] -> Pi^2/12 + (8 - 4*Log[a] + Log[a]^2)/2} ;
...
diage = Coefficient[total, e];
diagem1 = Coefficient[total, e^-1];
diagem2 = Coefficient[total, e^-2];
diag0 = total - e diage - diagem1/e - diagem2/e^2 // Expand;

diagtotal = {diagem2,diagem1,diag0 + e diage} // Expand;
...
Table[Print["Entry relative to ", c, " : \n", Expand[ 2 Simplify[ Coefficient[diagtotal[[2]], c] * (1/2/(sp[Ep1,Ep2] sp[q1,q2]-sp[q1,Ep2] sp[q2,Ep1])) ] + If[c === cB,HWF,0] ],"\n"],{c,{cB,cW,cWB}}];
...
Do[WriteString[FILE,ToString[c]<>"entry:=\n"];
   Write[FILE, Expand[ 2 Simplify[ Coefficient[diagtotal[[2]], c]* (1/2/(sp[Ep1,Ep2] sp[q1,q2]-sp[q1,Ep2] sp[q2,Ep1])) ] + If[c === cB,HWF,0] ] ];
   WriteString[FILE,"\n"];
   ,{c,{cB,cW,cWB}}];
WriteString[FILE,"\n"];WriteString[FILE,"\n"];
