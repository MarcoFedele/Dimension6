...
Paint[DiagramExtract[ins1, finaldiag], DisplayFunction -> (Export["WWHH_included.pdf",#]&), ColumnsXRows ->{3,4}];
Print["Included diagrams printed"];
If[Length[DiagramComplement[ins1, DiagramExtract[ins1, finaldiag]]] != 0,
   Paint[DiagramComplement[ins1, DiagramExtract[ins1, finaldiag]], DisplayFunction -> (Export["WWHH_excluded.pdf",#]&), ColumnsXRows ->{3,4}];
...
Paint[DiagramExtract[DiagramExtract[ins1, finaldiag], Flatten[{{1, 2}, Table[i, {i, 15, 27}]}] ] , DisplayFunction -> (Export["WWHH_prova.pdf",#]&), ColumnsXRows ->{3,4}];
temp = {0};
Do[temp = Append[temp, amp2[[i]]], {i, Flatten[{{1, 2}, Table[i, {i, 15, 27}]}]} ];
amp2 = Rest[temp];


FILE = NotebookDirectory[]<>"WWHHdiag.res";
