...
Paint[DiagramExtract[ins1, finaldiag], DisplayFunction -> (Export["WBHH_included.pdf",#]&), ColumnsXRows ->{3,4}];
Print["Included diagrams printed"];
If[Length[DiagramComplement[ins1, DiagramExtract[ins1, finaldiag]]] != 0,
   Paint[DiagramComplement[ins1, DiagramExtract[ins1, finaldiag]], DisplayFunction -> (Export["WBHH_excluded.pdf",#]&), ColumnsXRows ->{3,4}];
...
Paint[DiagramExtract[DiagramExtract[ins1, finaldiag], Join[Table[i, {i, 5, 6}], Table[j, {j,11, 12}]] ] , DisplayFunction -> (Export["WBHH_prova.pdf",#]&), ColumnsXRows ->{3,4}];
temp = {0};
Do[temp = Append[temp, amp2[[i]]], {i, Join[Table[i, {i, 5, 6}], Table[j, {j,11, 12}]] }];
amp2 = Rest[temp];


FILE = NotebookDirectory[]<>"WBHHdiag.res";
