...
Computeppspcounter[exp_] := Module[{ppcounter, spcounter, ppspcounter, diag, checkdiag},
                                   ppcounter = {0};
                                   spcounter = {0};
                                   diag = exp;
                                   ...
                                   Do[temp = 0;
                                      If[checkdiag == {1},
                                         If[MatchQ[diag[[DIAG]], 0],
                                            Print["There's something wrong in ppcounter!"],
                                            Do[i = 1;
                                               checkpp = 0;
                                               checksp = 0;
                                               While[i <= Length[diag[[DIAG, TERM]]],
                                                     If[MatchQ[diag[[DIAG, TERM, i]], pp[___, ___]], checkpp++];
                                                     If[MatchQ[diag[[DIAG, TERM, i]], sp[p, p]], checksp += 2];
                                                     If[MatchQ[diag[[DIAG, TERM, i]], sp[p, ___]], checksp++];
                                                     If[MatchQ[diag[[DIAG, TERM, i]], sp[p, ___]^2], checksp += 2];
                                                     If[MatchQ[diag[[DIAG, TERM, i]], p^2], checksp += 2];
                                                     i++];
                                               ppcounter = Append[ppcounter, checkpp];
                                               spcounter = Append[spcounter, checksp],
                                               {TERM, 1, Length[diag[[DIAG]]]}
                                               ];
                                            ],
                                         i = 1;
                                         checkpp = 0;
                                         ...
                                         While[i <= Length[diag[[DIAG]]],
                                               If[MatchQ[diag[[DIAG, i]], pp[___, ___]], checkpp++];
                                               If[MatchQ[diag[[DIAG, i]], sp[p, ___]], checksp++];
                                               If[MatchQ[diag[[DIAG, i]], sp[p, ___]^2], checksp += 2];
                                               If[MatchQ[diag[[DIAG, i]], p^2], checksp += 2];
                                               i++];
                                         ppcounter = Append[ppcounter, checkpp];
                                         spcounter = Append[spcounter, checksp];
                                         ],
                                   ...
                                   ppspcounter =
                                   Table[Table[{ppcounter[[j, i]], spcounter[[j, i]]}, {i, 1,Length[ppcounter[[j]]]}], {j, 1, Length[ppcounter]}];
                                   Return[ppspcounter];
                                   ];
