...
col = Complement[colore, {aaa}];
If[Length[col] === 1, diag = diag col[[1]] ];
...
(*diag = diag /. {pp[p, 0] pp[p - q1, 0] pp[p - q3, h] pp[p + q2 - q3, h] sp[p, Ep1] sp[p, Ep2] sp[p, q123] ->
                    pp[p, 0] pp[p + q1, 0] pp[p + q123, h] pp[p + q1 - q3, h] sp[p + q1, Ep1] sp[p + q1, Ep2] sp[p + q1, q123],
                pp[p, 0] pp[p - q1, 0] pp[p - q3, h] pp[p + q2 - q3, h] sp[p, Ep1] sp[p, Ep2] sp[p, q2] ->
                    pp[p, h] pp[p + q2, h] pp[p + q3, 0] pp[p - q1 + q3, 0] sp[p + q3, Ep1] sp[p + q3, Ep2] sp[p + q3, q2]};
...
diag = diag /. {pp[p, 0] pp[p - q1, 0] pp[p - q3, h] pp[p + q2 - q3, h] sp[p, Ep1] sp[p, Ep2] sp[p, q123] ->
    pp[p, 0] pp[p - q1, 0] pp[p - q3, h] pp[p + q2 - q3, h] sp[p, Ep1] sp[p, Ep2],
    pp[p, 0] pp[p - q1, 0] pp[p - q3, h] pp[p + q2 - q3, h] sp[p, Ep1] sp[p, Ep2] sp[p, q2] ->
    pp[p, 0] pp[p - q1, 0] pp[p - q3, h] pp[p + q2 - q3, h] sp[p, Ep1] sp[p, Ep2]};
...
Module[{temp, checkdiag, count, temp0, temp1},
       ...
       Do[
                                     ...
                                     If[count[[DIAG,TERM]] == {3, 2},
                                        ...
                                        If[count[[DIAG, TERM]] == {4, 0},
                                           ...
                                           If[count[[DIAG, TERM]] == {4, 1},
                                              ...
                                              temp1 = diagSimplify[{temp1}] /. {Power[q1, n_] :> 0 /; n > 0, Power[q2, n_] :> 0 /; n > 0} // Simplify // Expand ;
                                              temp1 = temp1 //. PVsub;
                                              temp0 = Append[temp0, 0],
                                              If[count[[DIAG, TERM]] == {4, 2},
                                                 temp1 = ExpandSProducts[temp[[DIAG, TERM]] /. rule42];
                                                 temp1 = diagSimplify[{temp1}] /. {Power[q1, n_] :> 0 /; n > 0, Power[q2, n_] :> 0 /; n > 0} // Simplify // Expand ;
                                                 temp1 = temp1 //. PVsub;
                                                 temp0 = Append[temp0, 0]
                                                 ]]]]]]]]]]]
                               ...
                               If[count[[DIAG]][[1]] == {3, 1},
                                  ...
                                  If[count[[DIAG]][[1]] == {3, 2},
                                     ...
                                     If[count[[DIAG]][[1]] == {4, 0},
                                        ...
                                        temp0 = Append[temp0, temp1],
                                        If[count[[DIAG]][[1]] == {4, 1},
                                           temp1 = ExpandSProducts[temp[[DIAG]] /. rule41];
                                           temp1 = diagSimplify[{temp1}] /. {Power[q1, n_] :> 0 /; n > 0, Power[q2, n_] :> 0 /; n > 0} // Simplify // Expand ;
                                           temp1 = temp1 //. PVsub;
                                           temp0 = Append[temp0, temp1[[1]]],
                                           If[count[[DIAG]][[1]] == {4, 2},
                                              temp1 = ExpandSProducts[temp[[DIAG]] /. rule42];
                                              temp1 = diagSimplify[{temp1}] /. {Power[q1, n_] :> 0 /; n > 0, Power[q2, n_] :> 0 /; n > 0} // Simplify // Expand ;
                                              temp1 = temp1 //. PVsub;
                                              temp0 = Append[temp0, temp1[[1]]]
                                              ]]]]]]]]]]]
          ...
          status=PrintTemporary[" diag ",DIAG,"/",Length[checkdiag]," done"]
...
postPVdiag = diag;
diag = diag /. nd -> 4;
diag = diag /.{ A0[m_] -> m, B0[___] -> 1, C0[___] -> 0, D0[___] -> 0};
diag = Simplify[diag];
(*Print[""];
...
Print[""];
Print[""];
Print["ANOMALOUS DIMENSION ENTRIES FOR OWB."];
Print[""];
Print[""];
total = Total[diag];
Table[Print["Entry relative to ", c, " : \n", Expand[ -2 Simplify[ Coefficient[total, c (sp[Ep1, Ep2] sp[q1, q2] - sp[q1, Ep2] sp[q2, Ep1]) ] / 2 ] + If[c === cHWB,HWF,0] ],"\n"],{c,{cW, cHB, cHW, cHWB}}];
Print[""];
Print[""];
Print["ANOMALOUS DIMENSION ENTRIES FOR OWBtilde."];
Print[""];
Print[""];
total = Total[diag];
Table[Print["Entry relative to ", c, " : \n",
            Expand[ -2 Simplify[ Coefficient[total, c (LeviCivita[FourVector[q1], FourVector[q2], Index[Lorentz, 1], Index[Lorentz, 2]]sp[{Index[Lorentz,1]},Ep1] sp[{Index[Lorentz,2]},Ep2]) ] / 2
                                 ] + If[c === cHWBt,HWF,0] ],"\n"],{c,{cWt, cHBt, cHWt, cHWBt}}];

FILE = NotebookDirectory[]<>"WBHHad.res";
DeleteFile[FILE]; OpenWrite[FILE];
WriteString[FILE,"\n"];WriteString[FILE,"\n"];
WriteString[FILE,"########  ANOMALOUS DIMENSION ENTRIES FOR OWB  ########"];
WriteString[FILE,"\n"];WriteString[FILE,"\n"];
Do[WriteString[FILE,ToString[c]<>"entry:=\n"];
   Write[FILE, Expand[ -2 Simplify[ Coefficient[total, c (sp[Ep1, Ep2] sp[q1, q2] - sp[q1, Ep2] sp[q2, Ep1]) ] ] ] ];
   WriteString[FILE,"\n"];
   ,{c,{ceB22,ceB23,ceB32,ceB33,cuB22,cuB23,cuB32,cuB33,cdB22,cdB23,cdB32,cdB33,ceW22,ceW23,ceW32,ceW33,cuW22,cuW23,cuW32,cuW33,cdW22,cdW23,cdW32,cdW33}}];
WriteString[FILE,"\n"];WriteString[FILE,"\n"];
WriteString[FILE,"########  ANOMALOUS DIMENSION ENTRIES FOR OWB  ########"];
WriteString[FILE,"\n"];WriteString[FILE,"\n"];
Do[WriteString[FILE,ToString[c]<>"tentry:=\n"];
   Write[FILE, Expand[ -2 Simplify[ Coefficient[total, c (LeviCivita[FourVector[q1], FourVector[q2], Index[Lorentz, 1], Index[Lorentz, 2]]sp[{Index[Lorentz,1]},Ep1] sp[{Index[Lorentz,2]},Ep2]) ] ] ] ];
   WriteString[FILE,"\n"];
   ,{c,{ceB22,ceB23,ceB32,ceB33,cuB22,cuB23,cuB32,cuB33,cdB22,cdB23,cdB32,cdB33,ceW22,ceW23,ceW32,ceW33,cuW22,cuW23,cuW32,cuW33,cdW22,cdW23,cdW32,cdW33}}];
WriteString[FILE,"\n"];WriteString[FILE,"\n"];
