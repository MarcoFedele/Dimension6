...
Do[check = 0;
   temp = 0;
   Do[
      If[MatchQ[amp2[[DIAG,TERM]], Times[cHBOX/LAMBDA^2, ___]],
         temp += amp2[[DIAG,TERM]];
         check++;
         ],
...
Paint[DiagramExtract[ins1, finaldiag], DisplayFunction -> (Export["4H_included.pdf",#]&), ColumnsXRows ->{3,4}];
Print["Included diagrams printed"];
If[Length[DiagramComplement[ins1, DiagramExtract[ins1, finaldiag]]] != 0,
   Paint[DiagramComplement[ins1, DiagramExtract[ins1, finaldiag]], DisplayFunction -> (Export["4H_excluded.pdf",#]&), ColumnsXRows ->{3,4}];
...
FILE = NotebookDirectory[]<>"4Hdiag.res";
