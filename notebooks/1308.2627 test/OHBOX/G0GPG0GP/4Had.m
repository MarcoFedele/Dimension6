...
diag = diag/.{MH->Sqrt[h],MGP->Sqrt[h],MG0->Sqrt[h]};
...
diag = diag /. {q23^n_ -> sp[q2 - q3,q2-q3]^(n/2), q24^n_ -> sp[q2 - q4,q2-q4]^(n/2), q34^n_ -> sp[q3 + q4,q3+q4]^(n/2)};
diag = diag /. {q23 -> q2 - q3, q24 -> q2 - q4, q34 -> q3 + q4};
diag = ExpandScalarProducts[diag];
(*diag = diag /. {q2 q4 -> sp[q2, q4], q2 q3 -> sp[q2, q3], q3 q4 -> sp[q3, q4]};*)
diag = diag /. {B0[ - q1_ - q2_ , m1_ , m2_ ] -> B0[ q1 + q2 , m1 , m2 ], q1^2 -> h, q2^2 -> h, q3^2 -> h, q4^2 -> h};
...
Print[""];
total = Total[diag];
Table[Print["Entry relative to ", c, " : \n", Expand[ -2 Simplify[ Coefficient[total, c] * (-1) ] ],"\n"],{c,{cHBOX}}];


FILE = NotebookDirectory[]<>"4Had.res";
DeleteFile[FILE]; OpenWrite[FILE];
WriteString[FILE,"\n"];WriteString[FILE,"\n"];
WriteString[FILE,"########  ANOMALOUS DIMENSION ENTRIES  ########"];
WriteString[FILE,"\n"];WriteString[FILE,"\n"];
Do[WriteString[FILE,ToString[c]<>"entry:=\n"];
   Write[FILE, Expand[ -2 Simplify[ Coefficient[total,c]* (-1) ] ] ];
   WriteString[FILE,"\n"];
   ,{c,{cHBOX}}];
WriteString[FILE,"\n"];WriteString[FILE,"\n"];
