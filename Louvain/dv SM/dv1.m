(* Here I generate the diagrams for the H tadpole in the SM, using the background field method and
...
top = CreateTopologies[1, 1 -> 0, Adjacencies -> {3, 4, 5}, ExcludeTopologies -> {Internal} ];

ins1 = InsertFields[top,
                    process1,
                    Model -> "Mine/1505.02646.bgf.3",
                    GenericModel -> "Mine/1505.02646.bgf.3",
                    InsertionLevel -> {Particles}, ExcludeParticles -> {F[1|2|3|4, {1, ___}], F[1|2|3|4, {2, ___}], F[1|2|4, {3, ___}]}  ];
...
(* Here I print the diagrams that I will compute in dv_included.pdf, and the diagrams that I will
   not compute in dv_excluded.pdf. *)

Print["Diagrams included: ",finaldiag];
Print["Total number of diagrams included: ",Length[diag],"/",Length[amp0]];
Paint[DiagramExtract[ins1, finaldiag], DisplayFunction -> (Export["dv_included.pdf",#]&), ColumnsXRows ->{3,4}];
Print["Included diagrams printed.\n"];
If[Length[DiagramComplement[ins1, DiagramExtract[ins1, finaldiag]]] != 0,
   Paint[DiagramComplement[ins1, DiagramExtract[ins1, finaldiag]], DisplayFunction -> (Export["dv_excluded.pdf",#]&), ColumnsXRows ->{3,4}];
   Print["Excluded diagrams printed."]
   ];

(* Here I store the result I obtained so far in the file dv1.res. I have to do so, since I have to quit
   the kernel I am working on before loading the packages needed to go on, which is in conflict with
   FeynArts. *)

FILE = NotebookDirectory[]<>"dv1.res";
