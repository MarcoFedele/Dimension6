...
(* Here I print the diagrams that I will compute in az_included.pdf, and the diagrams that I will
   not compute in az_excluded.pdf. *)

Print["Diagrams included: ",finaldiag];
Print["Total number of diagrams included: ",Length[diag],"/",Length[amp0]];
Paint[DiagramExtract[ins1, finaldiag], DisplayFunction -> (Export["az_included.pdf",#]&), ColumnsXRows ->{3,4}];
Print["Included diagrams printed.\n"];
If[Length[DiagramComplement[ins1, DiagramExtract[ins1, finaldiag]]] != 0,
   Paint[DiagramComplement[ins1, DiagramExtract[ins1, finaldiag]], DisplayFunction -> (Export["az_excluded.pdf",#]&), ColumnsXRows ->{3,4}];
   Print["Excluded diagrams printed."]
   ];

(* Here I store the result I obtained so far in the file az1.res. I have to do so, since I have to quit
   the kernel I am working on before loading the packages needed to go on, which is in conflict with
   FeynArts. *)

FILE = NotebookDirectory[]<>"az1.res";
