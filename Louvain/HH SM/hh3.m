...
result = Collect[Total[diagfin] /. w -> gw^2 vev^2/4 /. z -> (g1^2 + gw^2) vev^2/4
                 ,{\[Epsilon], g^\[Alpha]\[Beta], e, B0[___], Log[_]}
                 , Simplify]//. (g1^2 + gw^2) vev^2 -> 4 z //. gw^2 vev^2 -> 4 w;
...
dresult = Collect[-D[result, AA]/2 /. AA -> h //. DB0sub //. B0simplify
                     ,{\[Epsilon], g^\[Alpha]\[Beta], e, B0[___], Log[_]}
                     , Simplify];

Print[dresult];

FILE = NotebookDirectory[]<>"ResultHH.res";

DeleteFile[FILE]; OpenWrite[FILE];
WriteString[FILE, "diagfin=\n"];
Write[FILE, diagfin];
WriteString[FILE, "resultHH=\n"];
Write[FILE, dresult];
Close[FILE];
